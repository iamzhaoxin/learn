import json
from ..items import *

# TODO 修改id和name
id = 4202
name = '荆州博物馆'


class M4202(scrapy.Spider):
    # TODO 修改spider_name和start_urls
    name = '4202'
    start_urls = ['http://www.jzmsm.org/yk/']
    custom_settings = {
        'ITEM_PIPELINES': {'Museum.pipelines.Pipeline': 300}
    }

    def parse(self, response, **kwargs):
        print("start " + name)
        col_id = int(str(id) + str(10000))
        # TODO 进入藏品目录页
        col_url = 'http://www.jzmsm.org/yk/cangpin/guobaoxinshang/'
        yield scrapy.Request(url=col_url, callback=self.cols_parse, meta={'col_id': col_id})

        exh_id = int(str(id) + str(10000))
        # TODO 进入展览目录页
        exh_url = 'http://www.jzmsm.org/yk/zhanlan/badachenlie/'
        yield scrapy.Request(url=exh_url, callback=self.exh_parse, meta={'exh_id': exh_id})

    # TODO 进入藏品详情页 working
    def cols_parse(self, response):
        col_lists = list(set(response.xpath('/html/body/div[3]/ul//@href').extract()))
        for col_url in col_lists:
            response.meta['col_id'] += 1
            col_url = 'http://www.jzmsm.org/' + col_url
            yield scrapy.Request(url=col_url, callback=self.col_parse, meta={'col_id': response.meta['col_id']})
        return

    # 藏品详情页
    def col_parse(self, response):
        item = Item()
        item['exh_id'] = item['exh_name'] = item['exh_info'] = item['exh_picture'] = item['exh_time'] = ''
        item['mus_name'] = name
        item['mus_id'] = id
        item['col_id'] = response.meta['col_id']
        # print(response)

        # TODO 数据分析、持久化存储
        item['col_name'] = response.xpath('/html/body/div[6]/div/div[1]/center/h1/text()')[0].extract().strip()
        item['col_info'] = response.xpath('/html/body/div[6]/div/div[3]')[0].xpath('string(.)')[0].extract().strip()
        item['col_era'] = item['col_name']
        item['col_picture'] = response.xpath('/html/body/div[6]/div//@src')[0].extract()
        yield item
        # print(item['col_name']+"  "+item['col_info']+"   "+item['col_picture']+"   "+item['col_era'])
        print("正在爬取藏品 " + item['col_name'] + " ing")
        return


    # 展览详情页
    def exh_parse(self, response):
        exhsss=response.xpath('/html/body/div[3]/table')
        exh_id=response.meta['exh_id']
        for exhss in exhsss:
            exh_id+=50
            exhs=exhss.xpath('.//ul')
            for exh in exhs:
                exh_id+=1
                item = Item()
                item['col_id'] = item['col_name'] = item['col_info'] = item['col_era'] = item['col_picture'] = ''
                item['mus_name'] = name
                item['mus_id'] = id
                item['exh_id'] = exh_id
                # print(response)

                # TODO 解析展览数据并持久化存储
                item['exh_name'] = response.xpath('.//a[@class="zl05"]/text()')[0].extract().strip()
                item['exh_info'] = response.xpath('.//*[@class="zl06"]')[0].xpath('string(.)')[
                    0].extract().strip()
                item['exh_picture'] = "http://www.jzmsm.org/"+response.xpath('.//img/@src')[0].extract().strip()
                item['exh_time'] = '常设'
                yield item
                # print(item['exh_picture'] + item['exh_name'] + item['exh_info'] + item['exh_time'])
                print("                                                 正在爬取展览 " + item['exh_name'] + " ing")
        return
