...
class Gmc1103Spider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        for exh_url in exh_urls:
            yield scrapy.Request(url=exh_url, callback=self.exh_parse)
    ...
    def col_parse(self, response):
        ...
        item['col_info'] = response.xpath('/html/body/div[3]/div[2]/div[2]/div[3]')[0].xpath('string(.)')[0].extract().strip()
        item['col_era'] = '近代'
        item['col_picture']="http://www.luxunmuseum.com.cn/" + response.xpath('/html/body/div[3]/div[2]/div[2]//@src')[0].extract()
